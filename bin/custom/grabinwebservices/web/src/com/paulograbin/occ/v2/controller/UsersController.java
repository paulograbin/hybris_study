...
import com.paulograbin.occ.constants.YcommercewebservicesConstants;
import com.paulograbin.occ.populator.HttpRequestCustomerDataPopulator;
import com.paulograbin.occ.queues.data.mostpurchased.MostPurchasedProductsWsDTO;
...
public class UsersController extends BaseCommerceController
{
	...
	@RequestMapping(value = "/{userId}/mostpurchasedproducts", method = RequestMethod.GET)
	@ResponseStatus(value = HttpStatus.OK)
	@ResponseBody
	public MostPurchasedProductsWsDTO getMostPurchasedProductsByCustomer(@PathVariable("userId") final String userId,
																		 @RequestParam(required = false, defaultValue = DEFAULT_FIELD_SET) final String fields) {
		if (isFalse(baseStoreService.getCurrentBaseStore().isMostPurchasedProductsFeatureSwitch())) {
			LOG.info("Most purchased products feature is disabled.");
			return new MostPurchasedProductsWsDTO();
		}

		LOG.debug("Fetching must purchased products for customer " + userId);

		final MostPurchasedProductsData mostPurchasedProductsData = mostPurchasedProductsFacade.fetchMostPurchasedProductsByCustomer(userId);

		return getDataMapper().map(mostPurchasedProductsData, MostPurchasedProductsWsDTO.class, fields);
	}
